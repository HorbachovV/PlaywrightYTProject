...
test(
	"Create DELETE API Request Test",
	{ tag: ["@PlaywrightWithJenkins"] },
	async ({ request }) => {
		...
		expect(jsonPOSTAPIResponse.booking.bookingdates).toHaveProperty(
			"checkin"
		);
		expect(jsonPOSTAPIResponse.booking.bookingdates).toHaveProperty(
			"checkout"
		...
		expect(jsonPOSTAPIResponse.booking.bookingdates.checkin).toBe(
			"2025-01-25"
		);
		...
		console.log(
			"DELETE API Response : " + (await deleteAPIResponse.body())
		);
	}
);
